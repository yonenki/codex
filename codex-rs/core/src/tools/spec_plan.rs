...
const ACP_AGENT_NAMESPACE: &str = "acp";
...
fn add_collaboration_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if collab_tools_enabled(turn_context, context.model_info) {
        if multi_agent_v2_enabled(turn_context) {
            ...
            registry.register_trusted_with_exposure(
                multi_agent_v2_handler(SpawnAcpAgentHandler, Some(ACP_AGENT_NAMESPACE)),
                exposure,
            );
            registry.register_trusted_with_exposure(
                multi_agent_v2_handler(MessageAcpAgentHandler, Some(ACP_AGENT_NAMESPACE)),
                exposure,
            );
            registry.register_trusted_with_exposure(
                multi_agent_v2_handler(FollowupAcpAgentHandler, Some(ACP_AGENT_NAMESPACE)),
                exposure,
            );
...
fn add_team_tools(
    ...
    if crate::guardian::is_basic_session_source(&context.turn_context.session_source)
        || !multi_agent_v2_enabled(context.turn_context)
    {
        return;
    }
